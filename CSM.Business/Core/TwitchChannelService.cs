...
namespace CSM.Business.Core
{
    internal class TwitchChannelService(IUserConfigDomain userConfigDomain, ILogger<TwitchChannelService> logger) : ITwitchChannelService
    {
        ...
        private Task TwitchClient_OnConnectionError(object? sender, OnConnectionErrorArgs e)
        {
            logger.LogError("Twitch client connection error: {error}", e.Error.ToString());
            return Task.CompletedTask;
        }

        private Task TwitchClient_OnConnected(object? sender, OnConnectedEventArgs e)
        {
        ...
        private Task TwitchClient_OnJoinedChannel(object? sender, OnJoinedChannelArgs e)
        {
        ...
        private Task TwitchClient_OnLeftChannel(object? sender, OnLeftChannelArgs e)
        {
        ...
        private Task TwitchClient_OnMessageReceived(object? sender, OnMessageReceivedArgs e)
        {
        ...
        private Task TwitchClient_OnChatCommandReceived(object? sender, OnChatCommandReceivedArgs e)
        {
            if (e.ChatMessage.Message.StartsWith("! ")) return Task.CompletedTask;
            if (e.Command.ArgumentsAsList.Count == 0) return Task.CompletedTask;
